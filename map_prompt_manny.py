...
class Map_Generator:
    def __init__(self, persona_name, base_map_json):
        if "name" not in base_map_json:
            raise KeyError(
                f"Base map must start with exactly one origin location (and its coordinates)."
            )
        self.map_json = base_map_json
        self.persona_name = persona_name
        self.check_object_prompt = """
        Is this thing a Place or Object? If Object, return 1 for True. If Place,
        return 0 for False. If unsure, return 0. Limit response to 1 or 0.
        """
    ...
    def check_object(self):
        return int(
            self.ChatGPT_request(
                f"{self.map_json['name']}: " + self.check_object_prompt
            )
        )
    ...
    def generate_children(self):
        self.children_string = self.ChatGPT_request(
            f"Consider the following place: {self.map_json['name']}. "
            + self.get_children_prompt
        )
        print(self.children_string)

    def get_relative_sizes(self):
        self.size_allocation_dict = self.ChatGPT_request(
            self.children_string + self.assign_size_prompt
        )
    ...
    def get_children_json(self):
        ...
        """
        architect_response = self.ChatGPT_request(
            parent_children_description + self.architect_prompt
        )
        ...
        assert (
            self.architect_response_json["layout"]["coordinates"]
            == self.map_json["coordinates"]
        )
    ...
    def get_dimensions(self, current_map, place):
        ...
        Return the dimensions as a dictionary with 'width' and 'height'. """
            + """
        Ex: {"width": 5, "height": 3}
        ONLY RETURN THE DICTIONARY. OMIT ALL ```json``` AND DO NOT RETURN
        ANYTHING ELSE. ONLY RETURN THE DICTIONARY.
        """
        )
    ...
    def create_map(self):
        ...
        if "children" not in self.map_json:
            ...
            self.map_json["children"] = self.architect_response_json["layout"][
                "children"
            ]
    ...
    def add_locations(self, persona: Persona = None, n_activities=1):
        """
        Takes in a persona and a base map json and prompts the persona to
        give a location that they would like to visit based on activity that
        ...
        # activities_batch = None
        while activities_batch is None:
            try:
                response = persona.generate_activities_batch(n_activities)
                activities_batch = json.loads(response)
            except:
                print("Error in generating activities. Trying again.")
                time.sleep(5)

        activities = activities_batch["activities"]
        reasonings = activities_batch["reasoning"].split(".")
        ...
        get_location_prompt = (
            "You are now the persona: "
            + persona["first"]
            + " "
            + persona["last"]
            + ".\n\nYou want to do the following activity: {}. \n\nBased on what you want to do and the following map, is it feasible for you to accomplish this activity? Think of a number on a scale of 1 to 10, with 1 being impossible and 10 being very possible. If it is more than a 7, think of the location on the existing map where you would like to do this activity. If it is less than a 7, think of a new location that could be added to the map to make it feasible for you to do this activity and limit the name to no more than 4 words. Here is the current map in JSON format:\n\n"
        )
        for activity, reasoning in zip(activities, reasonings):
            print(f"Activity: {activity}")
            print(f"Reasoning: {reasoning}")
            location_name = self.ChatGPT_request(
                get_location_prompt.format(activity)
                + json.dumps(map_json, indent=4)
                + "\n\nONLY RETURN THE NAME OF THE PLACE WHERE YOU WOULD LIKE TO DO THE ACTIVITY. DO NOT RETURN ANYTHING ELSE. OMIT ANY ```json``` AND DO NOT RETURN ANYTHING ELSE."
            )
            # location_name = "Library"

            if any(
                child["name"].lower() == location_name.lower()
                for child in map_json["children"]
            ):
                print(
                    f"{location_name} already exists in the map. Can already do activity: {activity}."
                )
                continue
            ...
            while size is None:
                try:
                    size = self.get_dimensions(map_json, location_name)
                except:
                    print("Error in getting dimensions. Trying again.")
                    time.sleep(5)

            activity_base_map = None
            while activity_base_map is None:
                try:
                    response = self.ChatGPT_request(
                        f"Based on the size ({size}) of the location, {location_name}, what should its coordinates be in this map:\n\n"
                        + json.dumps(self.map_json)
                        + f"\n\nMAKE SURE THAT THIS LOCATION DOES NOT OVERLAP WITH ANY OTHER LOCATIONS. ONLY RETURN THE COORDINATES and in the following format:"
                        + "{"
                        + f'"name": "{location_name}", "coordinates":[bottom_left_corner_x, bottom_left_corner_y, top_right_corner_x, top_right_corner_y]'
                        + "}"
                        + "DO NOT RETURN ANYTHING OTHER THAN THIS TEXT. OMIT ANY ```json``` AND DO NOT RETURN ANYTHING ELSE."
                    )
                    print("activity_base_map response: ", response)
                    activity_base_map = json.loads(response)
                except:
                    print("Error in generating location. Trying again.")
                    time.sleep(5)

            print(f"Activity base map: {activity_base_map}")
            map_generator = Map_Generator(
                persona["first"] + persona["last"], activity_base_map
            )
            # returns the children of the base map
            map_generator.create_map()

            map_json["children"].append(map_generator.map_json)
            self.map_json = map_json

            print(f"Added location: {location_name}")

        return persona, self.map_json
...
def experiment_3(persona_name, base_map_json, n_steps):
    os.makedirs(persona_name, exist_ok=True)
    base_map = base_map_json
    ...
    map_generator.fill_base_map()

    with open(f"{persona_name}/final_layout_step_{n_steps + 1}.json", "w") as f:
        json.dump(base_map, f, indent=4)
...
persona_name = "place"
base_map = {"name": "A Typical Place", "coordinates": [-50, -50, 50, 50]}
experiment_3(persona_name, base_map, 5)
