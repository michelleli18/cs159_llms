...
class Creator():
    ...
    def get_children_json(self, save=True):
        ...
        while True:
            ...
            try: 
                ...
                print(f"FAILED ATTEMPT AT GENERATING CHILDREN FOR {self.map_json['name']}.. TRYING AGAIN")
                continue
    ...
    def display_map(self, save=True):
        ...
        def recursive_draw(ax, item_json, color=(1,1,1), base=False):            
            if not 'coordinates' in item_json:
                return

            label = item_json['name']
            if base:
                label = ""
            
            add_rectangle(ax, label, item_json['coordinates'], color)
